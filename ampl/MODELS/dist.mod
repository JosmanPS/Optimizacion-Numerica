...
var Rprod 'regular-time production' {prod,fact} >= 0;
                # Regular-time production of each product
                # at each factory, in 1000s of cases
var Oprod 'overtime production' {prod,fact} >= 0;
...
###  OBJECTIVE  ###
minimize cost:  sum {p in prod, f in fact} rpc[p,f] * Rprod[p,f] +
                sum {p in prod, f in fact} opc[p,f] * Oprod[p,f] +
...
bal 'material balance' {p in prod, w in whse}:
    sum {(v,w) in rt}
       Ship [p,v,w] + (if w in fact then Rprod[p,w] + Oprod[p,w]) =
...
trdef 'transshipment definition' {p in prod, d in dctr}:
    Trans[p,d] >= sum {(d,w) in rt} Ship [p,d,w] -
                  (if d in fact then Rprod[p,d] + Oprod[p,d]);
