...
set prod 'products';    # Members of the product group

param pt 'production time' {prod} > 0;
                # Crew-hours to produce 1000 units
param pc 'production cost' {prod} > 0;
...
param dem 'demand' {prod,first..last+1} >= 0;
                # Requirements (in 1000s)
                # to be met from current production and inventory
param pro 'promoted' {prod,first..last+1} logical;
...
var Rprod 'regular production' {prod,time} >= 0;
                # Production using regular-time labor, in 1000s
var Oprod 'overtime production' {prod,time} >= 0;
                # Production using overtime labor, in 1000s
var Inv 'inventory' {prod,time,1..life} >= 0;
...
var Short 'shortage' {prod,time} >= 0;
...
minimize cost:
    ...
    sum {t in time, p in prod} otr * cs * pt[p] * Oprod[p,t] +
    sum {t in time, p in prod, a in 1..life} cri[p] * pc[p] * Inv[p,t,a] +
    sum {t in time, p in prod} crs[p] * pc[p] * Short[p,t];
...
rlim 'regular-time limit' {t in time}:
    sum {p in prod} pt[p] * Rprod[p,t] <= sl * dpp[t] * Crews[t];
...
dreq1 'first demand requirement' {p in prod}:
    Rprod[p,first] + Oprod[p,first] + Short[p,first]
                             - Inv[p,first,1] = dem[p,first] less iinv[p];
dreq 'demand requirements' {p in prod, t in first+1..last}:
...
ireq 'inventory requirements' {p in prod, t in time}:
    sum {a in 1..life} Inv[p,t,a] + iil[p,t] >= minv[p,t];
                # Inventory in storage at end of period t
                # must meet specified minimum
izero 'impossible inventories' {p in prod, v in 1..life-1, a in v+1..life}:
    Inv[p,first+v-1,a] = 0;
...
ilim1 'new-inventory limits' {p in prod, t in time}:
    Inv[p,t,1] <= Rprod[p,t] + Oprod[p,t];
                # New inventory cannot exceed
                # production in the most recent period
ilim 'inventory limits' {p in prod, t in first+1..last, a in 2..life}:
